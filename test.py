...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        print("Category: {}\nPhrases: {}\nGuessed: {}\n".format(category,obscuredPhrase,', '.join(['{}'.format(c) for c in guessed])),end='\n')
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        ...
        if random_diff>self.difficulty:
            return True
        else:
            return False
    ...
    def getPossibleLetters(self,guessed):
        ...
        for letter in LETTERS:
            if (letter not in guessed) and (self.prizeMoney>VOWEL_COST):
                availabale.append(letter)
    ...
    def getMove(self,category, obscuredPhrase, guessed): 
        ...
        for letter in self.SORTED_FREQUENCIES:
            if letter in possible_letters:
                possible_sorted.append(letter)
            else:
                continue    
        if len(possible_letters)==0:
            return 'pass'    
        else:    
            good_or_bad=self.smartCoinFlip()   
            if good_or_bad==True:
                return possible_sorted[-1]
